...
def process_file(pyfile_name):
    ...
    if pyfile_str[0].startswith("'''") or pyfile_str[0].startswith("\"\"\""):
    ...
    for line in pyfile_str:
        ...
        if line.startswith('def '):
            ...
            if doc_line.startswith("    '''"):
                ...
                extract = extract_code("'''", comment_str, pyfile_str,
                                       line_num)
            if doc_line.startswith("    \"\"\""):
                comment_str = doc_line[7:]
                extract = extract_code("\"\"\"", comment_str, pyfile_str,
...
def process_output(meta_file, outfile_name, code_links):
    ...
    if len(meta_file['modules']) > 1:
        ...
        for meta_doc in meta_file['modules']:
            ...
            chapter_link = chapter_name.lstrip().replace('.', '').replace(
                ' ', '-').lower()
    ...
    doc_str += '*'
    doc_str += 'Generated by a tool forked from [py2md](https://github.com/gbowerman/py2md) on '
